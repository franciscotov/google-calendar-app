import type { BookingAppModel } from "../_hooks/useBookingApp";
...
type BookingAppViewProps = {
  model: BookingAppModel;
};

export function BookingAppView({ model }: BookingAppViewProps) {
  const {
    apiBaseUrl,
    googleClientId,
    googleButtonRef,
        ...
        {user && (
          <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
            <section className="space-y-6 rounded-3xl border border-[#dbe5dd] bg-white p-6 shadow-[0_20px_40px_-25px_rgba(28,46,38,0.5)] md:p-8">
              ...
              <ConnectCalendarCard
                calendarIdInput={calendarIdInput}
                connectedCalendarId={connectedCalendarId}
                defaultCalendarId={user.email || DEFAULT_CALENDAR_PLACEHOLDER}
                isBusy={isBusy}
                onCalendarIdChange={setCalendarIdInput}
            ...
            <AvailableSlotsCard availableSlots={availableSlots} onApplySlot={applySlotToForm} />
          </div>
        )}
